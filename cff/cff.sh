...
!   Blank out end of statement
      ...
      IF (LENST < LEN .AND. MOD(LENST, $flen6) /= 0)                       &
          STAMNT(LENST+1: LENST+$flen6-MOD(LENST, $flen6)) = ' '
!
99 RETURN
...
!
      ...
      CHARACTER(LEN=$flen6) :: FIELD
      CHARACTER(LEN=$flen) :: LINE
      CHARACTER(LEN=$flen), PARAMETER :: BLANKV=' '
!
...
!   Check whether a comment line and if so copy to buffer.
      ...
      USER .OR. LINE == ' ' .OR. LINE(:1) == 'c'                       &
         ...
         IF (KNTCOM == KKLIM) THEN
            WRITE (NOUT , '(A$flen)') (CBUF($flen*L5-$flen1:$flen*L5) , L5 = 1 ,    &
            KNTCOM) , LINE
            KNTCOM = 0
         ELSE IF (SYNERR .OR. .NOT.STAT) THEN
            WRITE (NOUT , '(A$flen)') LINE
         ELSE
            KNTCOM = KNTCOM+1
            CBUF($flen*KNTCOM-$flen1:$flen*KNTCOM) = LINE
         END IF
         GO TO 2
      END IF
!
!   Some form of embedded comment?
      ...
      DO L22 = 2, $flen
         ...
         IF (.NOT. INTFL) THEN
            IF (KNTCOM  <  KKLIM) THEN
               KNTCOM = KNTCOM +1
               CBUF($flen*KNTCOM-$flen1:$flen*KNTCOM) =                          &
                                BLANKV(:L22-1)//LINE(L22:$flen)
            ELSE
               WRITE (NOUT, '(A)') BLANKV(:L22-1)//LINE(L22:$flen)
            END IF
         END IF
         LINE(L22:$flen) = ' '
         IF (LINE  ==  ' ') GO TO 2
...
!   Line is some form of statement; re-read.
      READ (LINE , '(BN , I5 , A1 , A$flen6)') LAB , CONTIN , FIELD
      STAT = .TRUE.
!
!   Check on syntax and copy to statement buffer
    3 IF (CONTIN == '0') CONTIN = ' '
      IF (CONTIN /= ' ') THEN
         ...
         IF (SYNERR) THEN
            GO TO 6
         ELSE IF (LENST == 0 .OR. LENST+$flen6 > LEN .OR. LAB /= 0) THEN
            SYNERR = .TRUE.
            IF (LENST > 0) THEN
               IF (LABEL /= 0) THEN
                  WRITE (NOUT , '(I5, 1X, A$flen6:"&"/(5X, "&", A$flen6:       &
     &            "&"))') LABEL ,                                      &
                  (STAMNT($flen6*L9-65:$flen6*L9) , L9 = 1 , (LENST+65)/$flen6)
               ELSE
                  WRITE (NOUT , '(6X, A$flen6:"&"/(5X, "&", A$flen6:"&"        &
     &            ))') (STAMNT($flen6*L9-65:$flen6*L9) , L9 = 1 , (LENST+65)/$flen6)
               END IF
            END IF
            IF (LAB /= 0) THEN
         ...
         ELSE
            KNTCON = KNTCON+1
            STAMNT(LENST+1:LENST+$flen6) = FIELD
            LENST = LENST+$flen6
            GO TO 2
         END IF
      ELSE IF (KNTCON == 0) THEN
         IF (LENST /= 0) GO TO 4
         STAMNT(1:$flen6) = FIELD
         LENST = $flen6
         LABEL = LAB
         IF (SYNERR) GO TO 4
         GO TO 2
...
!   Set counters for new program unit
               ...
               KNTCON = 0
               LENST = 0
               IF (KNTCOM /= 0) WRITE (NOUT , '(A$flen)') (CBUF($flen*L5-$flen1: &
               $flen*L5) , L5 = 1 , KNTCOM)
               KNTCOM = 0
               NAME = ' '
               NOARG = 0
...
!
   ...
   implicit none
!
      INTEGER, PARAMETER :: LLIMIT = LEN-(LEN/$flen6-1)*6
...
!   and the ** and // operators and real literal constants are treated.
    5 KADD = 0
      IF (L3-L2  <=  $flen6-MOD(IPNT , $flen6)) GO TO  8
      DO L4 = $flen6+L2-MOD(IPNT , $flen6) , L2 , -1
...
!   Set pointers for next section of statement
      IPNT = LOUT+1
      IF (KADD == 1 .AND. MOD(IPNT , $flen6) /= 1 .OR. MOD(IPNT , $flen6)      &
       >= 60) IPNT = ((IPNT+65)/$flen6)*$flen6+1
      IF (MOD(IPNT , $flen6) == 0) IPNT = IPNT+1
      JPNT = L3+1
...
!   total length to exceed 2640, must start it in col.1)
    9 LENST = LEN_TRIM(STAMNT(:LENST))
      ...
      END IF
      IF (LABEL /= 0) THEN
         WRITE (NOUT , 1003) LABEL , STAMNT(:MIN(LENST, $flen6)), AMP
      ELSE
         IF (LENST < LEN-6) THEN
            WRITE (NOUT , 1004) STAMNT(:MIN(LENST,$flen6)), AMP
         ELSE
            WRITE (NOUT , '(A,A1)') STAMNT(:MIN(LENST, $flen6)), AMP
         END IF
      END IF
      IF (LENST > $flen6) WRITE (NOUT , 1005)                              &
     &('&', STAMNT($flen6*L6-65:$flen6*L6) , L6 = 2 , (LENST+65)/$flen6)
      GO TO  11
!
!   Write OUT to output unit
   10 LOUT = LEN_TRIM(OUT(:LOUT))
      ...
      END IF
      IF (LABEL /= 0) THEN
         WRITE (NOUT , 1003) LABEL , OUT(:MIN(LOUT, $flen6)), AMP
      ELSE
         WRITE (NOUT , 1004) OUT(:MIN(LOUT, $flen6)), AMP
      END IF
!
...
!   compatible with both old and new source forms.
      IF (LOUT > $flen6) THEN
         ...
         ELSE
            AMP = ' '
         END IF
         WRITE (NOUT , 1005) (AMP , OUT($flen6*L5-65:$flen6*L5) , L5 = 2 , (   &
         LOUT+65)/$flen6)
      END IF
!
!   Write any comments following statement
   11 IF (KNTCOM /= 0) THEN
         WRITE (NOUT ,'(A$flen)') (CBUF($flen*L5-$flen1:$flen*L5) , L5 = 1 , KNTCOM)
         KNTCOM = 0
      END IF
!
